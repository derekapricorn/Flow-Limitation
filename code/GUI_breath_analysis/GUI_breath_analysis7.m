...
% Last Modified by GUIDE v2.5 26-Mar-2015 10:36:22
...
[filename, pathname] = uigetfile('*.mat', 'Pick a segment');
    if isequal(filename,0) || isequal(pathname,0)
       ...
       %find subject number
       if numel(filename) == 7
           handles.subject = str2double(filename(4));
       elseif numel(filename == 8)
           handles.subject = str2double(filename(4:5));
       end
...
% handles    structure with handles and user data (see GUIDATA)
fn = dir('*Data.txt');
[t_res,chest,abdom] = respitrace(fn.name);
axes(handles.axes4);
plot(t_res,chest,'b',t_res,abdom,'k')
guidata(hObject,handles);
...
%increment the file number name
handles.subject = handles.subject + 1;
fn = strcat('tp_',num2str(handles.subject),'.mat');
handles.filename = fn;
...
%on or now
handles.stage_on = 0; % stage_on = -1 =>"OFF", 1 =>"ON"
[y_ref,y_lim] = find_pk_ref(handles.pathname);
handles.y_lim = y_lim;
handles.y_ref = y_ref;
